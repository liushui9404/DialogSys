...
class Model:
    def __init__(self, input_steps, embedding_size, hidden_size, vocab_size, slot_size,
        ...
        self.input_steps = input_steps   # 展开步数
        self.embedding_size = embedding_size  # 词向量维数
        self.hidden_size = hidden_size     # 隐藏层节点个数
        self.n_layers = n_layers          # 隐藏层数
        self.batch_size = batch_size      # batch大小
        self.vocab_size = vocab_size      # 词典大小
        self.slot_size = slot_size       # 槽的大小？？？
        self.intent_size = intent_size   # 意图的种类
        self.epoch_num = epoch_num       # 训练几轮
        
        self.encoder_inputs = tf.placeholder(tf.int32, [input_steps, None],
                                             name='encoder_inputs')
        # 每句输入的实际长度，除了padding
        self.encoder_inputs_actual_length = tf.placeholder(tf.int32, [None],
        ...
        self.batch_size = tf.shape(self.encoder_inputs)[1]  # 这里是为了加载模型测试所用的，否则会因为batch_size不匹配而报错

    def build(self):
        
        # 提取出词向量矩阵
        file_i  = open('/home/liu/新论文/classify_W/skip-gram-nopos/embeddings','rb')
        self.embeddings = pickle.load(file_i)
        file_i.close()
        ...
        # 求intent
        intent_logits = tf.add(tf.matmul(encoder_final_state_h, intent_W), intent_b)   #这里仅仅用到最后的输出
        # intent_prob = tf.nn.softmax(intent_logits)
        self.intent = tf.argmax(intent_logits, axis=1,name ="pred_intent")
        ...
        tf.add_to_collection('pred_solt',outputs.sample_id)  # 为了之后读取模型所用
